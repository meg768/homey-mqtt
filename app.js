...
class MyApp extends Homey.App {
	async onInit() {
        ...
        for (let [deviceID, device] of Object.entries(devices)) {
			...
			if (settings.homekit_exclude !== true) {
		...
		this.log(`Cleaning up ${this.config.mqtt.host}:${this.config.mqtt.port}.`);
		await this.cleanUp();

		this.log(`Connecting to MQTT broker ${this.config.mqtt.host}:${this.config.mqtt.port}.`);

		this.mqtt = MqttAsync(
			Mqtt.connect(this.config.mqtt.host, {
	...
	async publish(topic, value, debug = true) {
		if (debug) this.debug(`Publishing ${topic}:${value}.`);
